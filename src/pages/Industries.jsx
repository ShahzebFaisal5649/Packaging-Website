  ...
  return (
    ...
    >
      ...
      <div style={{ height: 220, width: '100%', overflow: 'hidden', borderRadius: '12px 12px 0 0', background: '#F1EDE6', position: 'relative' }}>
        {/* Robust image field: supports image, img, thumbnail from DB */}
        {(() => { const imgUrl = product.image || product.img || product.thumbnail || ''; return imgUrl ? (
          <img
            src={imgUrl}
            alt={`${product.name} packaging — Design Custom Box`}
            style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.6s ease', transform: hovered ? 'scale(1.07)' : 'scale(1)' }}
            loading="lazy"
            onError={e => {
              e.target.onerror = null;
              e.target.src = 'https://images.unsplash.com/photo-1607082348824-0a96f2a4b9da?auto=format&fit=crop&w=600&h=400&q=80';
            }}
          />
        ) : (
          <div style={{
